import argparse, sys, os, ffmpeg, queue, asyncio, json, face_recognition, cv2, time, numpy as np, pickle
...
class FaceRipper:
	...
	def __init__(self, **kwargs):
		...
		self.blur_threshold = self.kwargs['blur']
		self.faces = []
		self.encodings = []
		self.queue = []
		self.threads = []
...
if __name__ == "__main__":
	...
	parser.add_argument('--tolerance', type=float, default=0.35, help='Tolerance for facial recognition, ranging from 0.0 - 1.0 where the lower the number the stricter the matching. The more reference images you have, I recommend lowering it. Default is 0.35 (35%)')
	...
	parser.add_argument('--blur', type=float, default=100, help='The blur threshold to use when getting rid of faces that are too blurry. Default is 100.')
